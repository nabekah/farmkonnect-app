...
import { Plus, Trash2, Play, Save, X, AlertCircle } from "lucide-react";
...
export function WorkflowBuilderIntegrated({ farmId }: { farmId: number }) {
  ...
  const executeWorkflow = trpc.workflowBuilder.executeWorkflow.useMutation();
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      await createWorkflow.mutateAsync({
        ...
        nodes: nodes.map(({ id, type, label, config, position }) => ({
          id,
          type,
          label,
          config,
  ...
  const testWorkflow = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Workflow executed successfully. Execution ID: ${result.executionId}`,
      });
    } catch (error) {
      toast({
        title: "Error",
